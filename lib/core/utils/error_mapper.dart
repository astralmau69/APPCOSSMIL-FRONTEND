...
class ErrorMapper {
  static String message(dynamic error, {ErrorContext context = ErrorContext.general}) {
    if (error == null) return 'Ocurrió un error inesperado. Intenta nuevamente.';

    final msg = error.toString();
    final lowerMsg = msg.toLowerCase();

    // 1. Errores de Red y Timeout
    if (error is SocketException || lowerMsg.contains('socket') || lowerMsg.contains('internet') || lowerMsg.contains('network')) {
    ...
    if (_containsStatusCode(lowerMsg, 401) || lowerMsg.contains('unauthorized') || lowerMsg.contains('unauthenticated')) {
    ...
    if (_containsStatusCode(lowerMsg, 500) || _containsStatusCode(lowerMsg, 502) || _containsStatusCode(lowerMsg, 503) || _containsStatusCode(lowerMsg, 504)) {
    ...
    if (lowerMsg.contains('dioexception') || lowerMsg.contains('bad response') || lowerMsg.contains('xmlhttprequest')) {
    ...
    if (cleanedMsg.isNotEmpty && !cleanedMsg.contains('{') && !cleanedMsg.contains('<') && cleanedMsg.length < 80) {
      if (lowerMsg.contains('inválido') || lowerMsg.contains('incorrecto') || lowerMsg.contains('credenciales')) {
  ...
  static bool _containsStatusCode(String lowerMsg, int code) {
    return lowerMsg.contains('$code') && 
          (lowerMsg.contains('status code') || lowerMsg.contains('error') || lowerMsg.contains('http'));
  }
}
