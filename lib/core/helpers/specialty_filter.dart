...
class SpecialtyFilter {
  ...
  static List<SpecialtyModel> apply({
    ...
    return specialties.where((s) {
      ...
      if (kDebugMode && (idesp == _idPediatria || idesp == _idGinecologia || nameUpper.contains('GERIATRIA') || nameUpper.contains('ODONTOPEDIATRIA'))) {
        debugPrint('🔎 SpecialtyFilter.apply: idesp=$idesp, name="${s.name}", age=$age, gender="$genderUpper"');
      }

      // Regla 1: Pediatría y Odontopediatría — Máximo 15 años
      final isPediatria = idesp == _idPediatria || nameUpper.contains('PEDIATRIA') || nameUpper.contains('ODONTOPEDIATRIA');
      if (isPediatria) {
        if (age > _edadMaxPediatria) {
          if (kDebugMode) debugPrint('   ❌ Ocultando $nameUpper (edad=$age > $_edadMaxPediatria)');
          return false;
        }
      }

      // Regla 2: Ginecología (31) — Solo para Femenino >= 13 años
      final isGinecologia = idesp == _idGinecologia || nameUpper.contains('GINECOLOGIA') || nameUpper.contains('GINECOLOGÍA');
      if (isGinecologia) {
      ...
      final isGeriatria = nameUpper.contains('GERIATRIA') || nameUpper.contains('GERIATRÍA');
      if (isGeriatria) {
        if (age < _edadMinGeriatria) {
          if (kDebugMode) debugPrint('   ❌ Ocultando Geriatría (edad=$age < $_edadMinGeriatria)');
          return false;
        }
      }
