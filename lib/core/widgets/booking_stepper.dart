...
class _LiquidProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final liquidGradient = LinearGradient(
      colors: isDark
          ? [const Color(0xFF005EB8), const Color(0xFF059669)]
          : [const Color(0xFF00478D), const Color(0xFF059669)],
    );
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, fillWidth, size.height), basePaint);
  ...
  void _drawBubbles(Canvas canvas, Size size, double fillWidth) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final offsetY = math.sin(wavePhase + i * 1.2) * 2;

      if (baseX < fillWidth - 3) {
        canvas.drawCircle(Offset(baseX, baseY + offsetY), radius, bubblePaint);
      }
    }
  }
