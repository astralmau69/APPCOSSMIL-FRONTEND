...
class _SplashScreenState extends State<SplashScreen>
  ...
  void _setupAnimations() {
    ...
    _taglineSlide = Tween<Offset>(begin: const Offset(0, 0.8), end: Offset.zero)
        .animate(
          CurvedAnimation(parent: _taglineCtrl, curve: Curves.easeOutQuint),
        );
  ...
  Future<void> _runSequence() async {
    ...
    // Hold final. En móvil se espera ~8s para que suene el audio de bienvenida.
    // En WEB el navegador bloquea el autoplay (no hay audio que esperar), así
    // que acortamos drásticamente: solo el tiempo para que las animaciones se
    // vean bien. Antes el splash web tardaba ~8s en vano.
    await Future.delayed(
      Duration(
        milliseconds: kIsWeb
            ? (widget.isOverlay ? 800 : 1200)
            : (widget.isOverlay ? 2000 : 6300),
      ),
    );
  ...
  Future<void> _checkPermissionsOnUpdate() async {
    ...
    final needsSettings =
        notifStatus.isPermanentlyDenied || locationStatus.isPermanentlyDenied;
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: (isDark ? AppColors.primaryMedium : AppColors.primary)
                    .withValues(
                      alpha: _rippleOpacity.value * opacityMultiplier,
                    ),
      ...
      child: Column(
        children: [
          Container(
            ...
            color: (isDark ? AppColors.white : AppColors.primary).withValues(
              alpha: 0.15,
            ),
          ),
