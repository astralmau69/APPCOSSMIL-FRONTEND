...
void main() {
  // ApiClient llama TokenStorage.getToken() (flutter_secure_storage, canal de
  // plataforma) antes de cada request → el binding de test debe existir y el
  ...
  group('ProgramacionService', () {
    ...
    test(
      'getRegionalesPorDepartamento lanza excepci\u00f3n en error de API',
      () async {
        ...
        final service = ProgramacionService(apiClient: _apiClient(client));
        expect(() => service.getRegionalesPorDepartamento(1), throwsException);
      },
    );
    ...
    test(
      'getFechaServidor lanza excepci\u00f3n si el formato es inv\u00e1lido',
      () async {
    ...
    test('crearCita retorna datos de la cita cuando ok=true', () async {
      ...
      final result = await service.crearCita(
        payload: {'idper': 123, 'idsuc': 1, 'idesp': 10, 'idhora': 'hora-001'},
      );
    ...
    test(
      'crearCita lanza excepci\u00f3n cuando ok=false en respuesta',
      () async {
        final client = _mockClient(200, {
          'ok': false,
    ...
    test(
      'getGrupoFamiliar retorna lista vac\u00eda si data es vac\u00eda',
      () async {
