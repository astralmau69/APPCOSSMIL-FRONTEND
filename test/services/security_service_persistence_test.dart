...
void main() {
  ...
  group('SecurityService · persistencia de biometría', () {
    test('La preferencia de biometría persiste tras habilitarla', () async {
      ...
      for (var i = 0; i < 5; i++) {
        expect(
          await SecurityService.isBiometricsEnabled(),
          isTrue,
  ...
  group('SecurityService · únicos casos que SÍ deben borrar la seguridad', () {
    test(
      'Cambio de cuenta / logout (clearSecurityData) borra PIN y huella',
      () async {
        await SecurityService.savePin('1234');
        await SecurityService.setBiometricsEnabled(true);
        expect(await SecurityService.hasPin(), isTrue);
  ...
  group('SecurityService · resiliencia (no perder el PIN por error transitorio)', () {
    test('Un fallo transitorio del Keystore NO borra ni oculta el PIN', () async {
      await SecurityService.savePin('1234');
      ...
      expect(
        await SecurityService.hasPin(),
        isTrue,
        reason: 'Un fallo transitorio no debe interpretarse como "sin PIN"',
      );
    });

    test(
      'Un fallo PERSISTENTE relanza la excepción (no asume "sin PIN")',
      () async {
        await SecurityService.savePin('1234');
  ...
  group(
    'SecurityService · isLocalAuthConfiguredSafe (nunca degradar al login)',
    () {
      ...
      test(
        'Si falla la lectura de la huella (sin PIN) → asume configurado',
        () async {
          failKeys.add(
            'use_biometrics',
          ); // sin PIN, pero la huella no se puede leer
          expect(await SecurityService.isLocalAuthConfiguredSafe(), isTrue);
        },
      );
    },
  ...
  group('SecurityService · bloqueo anti fuerza bruta del PIN', () {
    test(
      'Tras 5 intentos fallidos activa cooldown y reinicia la ventana',
      () async {
